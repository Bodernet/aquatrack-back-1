...
export const googleRedirect = async (req, res, next) => {
  try {
    ...
    // Assuming tokenData contains access_token and optionally refresh_token
    const { access_token, refresh_token } = tokenData.data;

    // Set cookie (example using cookie-parser middleware)
    ...
    if (!user) {
      ...
      const hashedPassword = await bcrypt.hash(given_name, 10); // Using a temporary password
      const newUser = new User({
        email,
        name: given_name,
        password: hashedPassword,
    ...
    const { token, tmpToken } = await tokenServices.generateToken(payload);
    await tokenServices.saveToken(user._id, tmpToken);
